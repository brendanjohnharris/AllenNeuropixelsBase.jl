...
abstract type AbstractSparseDimArray{T, N, D, A} <: AbstractToolsArray{T, N, D, A} end

struct SparseDimArray{T, N, D <: Tuple, R <: Tuple, A <: AbstractSparseArray{T, Int64, N},
                      Na, Me} <: AbstractSparseDimArray{T, N, D, A}
    data::A
    dims::D
    refdims::R
    name::Na
...
function DimensionalData.format(dims, x::AbstractSparseDimArray)
...
function SparseDimArray(data::A, dims::D, refdims::R = (),
...
function goSparseDimArray(data::A, dims::D, refdims::R = (),
                          name::Na = DimensionalData.NoName(),
                          metadata = DimensionalData.NoMetadata()) where {D, R, A, Na}
    @info "Constructing a sparsedimarray properly"
    SparseDimArray(data, DimensionalData.format(dims, axes(data)), refdims, name, metadata)
...
function DimensionalData.rebuild(A::AbstractSparseDimArray, data::AbstractSparseArray,
