...
function datapaths()
    data = getmanifest()
    all_files = reduce(merge, [data[k] for k in keys(data) if data[k] isa AbstractDict]; init=Dict{String,Any}())
    urls = [v isa AbstractDict ? v["url"] : all_files[k] for (k, v) in all_files]
    tree = filetree(String.(urls))
